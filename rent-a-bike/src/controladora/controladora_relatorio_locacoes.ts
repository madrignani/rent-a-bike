import type { VisaoRelatorioLocacoes } from "../visao/visao_relatorio_locacoes.ts";
...
export class ControladoraRelatorioLocacoes {
    private visao: VisaoRelatorioLocacoes;
    private gestorLogin = new GestorLogin();
    private gestorRelatorio = new GestorRelatorio();

    constructor(visao: VisaoRelatorioLocacoes) {
        this.visao = visao;
    }
    ...
    private async verificarPermissao(): Promise<void> {
        ...
        if (funcionario.getCargo() === "MECANICO" || funcionario.getCargo() === "ATENDENTE") {
            this.visao.exibirMensagem( ["Acesso restrito para mecânicos e atendentes."] );
    ...
    async gerarRelatorio(filtro: FiltroPeriodo): Promise<void> {
        try {
            const registros = await this.gestorRelatorio.obterLocacoesDevolvidas(filtro);
